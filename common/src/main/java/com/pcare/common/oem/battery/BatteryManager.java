package com.pcare.common.oem.battery;
			...
			readCharacteristic(mBatteryLevelCharacteristic)
					...
					.fail(new FailCallback() {
						...
						public void onRequestFailed(@NonNull BluetoothDevice device, int status) {
							BatteryManager.this.log(Log.WARN, "Battery Level characteristic not found");
						}
					})
					.enqueue();
		}
			...
			enableNotifications(mBatteryLevelCharacteristic)
					.done(new SuccessCallback() {
						...
						public void onRequestCompleted(@NonNull BluetoothDevice device) {
							BatteryManager.this.log(Log.INFO, "Battery Level notifications enabled");
						}
					})
					.fail(new FailCallback() {
						@Override
		...
		if (isConnected()) {
			disableNotifications(mBatteryLevelCharacteristic)
					.done(new SuccessCallback() {
						@Override
						public void onRequestCompleted(@NonNull BluetoothDevice device) {
							BatteryManager.this.log(Log.INFO, "Battery Level notifications disabled");
						}
					})
					.enqueue();
		}
